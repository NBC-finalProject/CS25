...
public class RagService {
    ...
    public void saveDocumentsToVectorStore(List<Document> docs) {
        ...
        for (Document doc : validDocs) {
            log.info("임베딩할 문서 경로: {}, 글자 수: {}", doc.getMetadata().get("path"), doc.getText().length());
            log.info("임베딩할 문서 내용(앞 100자): {}", doc.getText().substring(0, Math.min(doc.getText().length(), 100)));
        }
